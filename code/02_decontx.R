...
for (i in seq_along(samples)){
  assign(paste0("scs_data", samples[i]), Read10X(data.dir = paste0("/data/Project/skin-aging/cellranger/", samples[i], "/outs/filtered_feature_bc_matrix")))
}
...
for  (i in seq_along(samples)){
  ...
  save(list=scename,
       file=paste('/data/Project/skin-aging/2.ambiantRNAremoved_DecontX/sceFiles/',samples[i],'.rda',sep=''))
}
